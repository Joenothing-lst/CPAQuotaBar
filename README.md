...
采用原生 SwiftUI 与 AppKit 界面，支持账号池切换、双窗口额度圆环、请求统计和按阈值自动禁用账号。应用直接连接 CPA，无需安装额外的额度插件，也没有第三方 Swift 包依赖。
...
更新时，退出正在运行的 CPAQuotaBar，用新版 `.app` 覆盖旧应用，再重新打开。连接信息和本地策略会保留。
...
本项目代码采用 [MIT License](LICENSE)。服务商名称与品牌标识用于识别所连接的服务，相关商标权归各自权利人所有。本项目与 OpenAI、Google、Anthropic 没有隶属或背书关系。
